...
**Total: 100 harnesses.** Per-harness self-test and proof status is generated locally by
...
Deferred OWASP coverage closeout: the previously-deferred A08 (Software/Data
Integrity Failures), LLM04 (Data Poisoning), LLM07 (System Prompt Leakage), and
LLM09 (Misinformation) were built and merged in Batch 11 (see below). No OWASP
Top 10:2025 (web) or LLM 2025 categories remain deferred.

## Batch 11 — Exploratory Proof Layer + deferred-OWASP closeout (#93–100, shipped 2026-06-27)

Eight harnesses: four core exploratory-proof harnesses plus the four
previously-deferred OWASP categories. All declare `TEETH` (required); each ships
a paired unittest and a planted-bad proof test.

**core/ — Exploratory Proof Layer**
- `combinatorial_coverage` — t-way / combinatorial input-coverage measurement.
- `counterexample_replay` — replay of recorded counterexamples as regression seeds.
- `stateful_sequence_budget` — budgeted stateful operation-sequence exploration.
- `boundary_corpus_expander` — boundary-value corpus expansion.

**security/ — OWASP Top 10:2025 (web)**
- `data_integrity` — A08 Software/Data Integrity Failures.

**ai/ — OWASP Top 10 for LLM Applications 2025**
- `data_poisoning` — LLM04 Data Poisoning.
...
**100 harnesses.** Live per-harness self-test and proof status is in `STATUS.md`
