...
- [`CLAUDE.md`](./CLAUDE.md) — contributor guide, harness pattern, port table, add-a-harness procedure.

## Status handling

`STATUS.md` is generated by `make report` and uploaded by CI as an artifact. If a committed `STATUS.md` is absent, run:

```bash
make report
```

This avoids treating a stale generated report as canonical. The source of truth is the harness code, paired tests, and CI/test output.
