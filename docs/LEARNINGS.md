...
- Follow-up (same day): extended `_DEAD_VALUE_TYPES` handling to flag a bare `ast.Tuple` — but only when every element is itself side-effect-free by shape, so the assert-tuple / trailing-comma footgun (`x == 1, "msg"` or `foo,`, a bare tuple that is always truthy and discarded) is caught while an intentional `(do_a(), do_b())` is not. Self-test now covers 8 kinds; repo stays 69/69 clean. `ast.List/Set/Dict/Lambda` were considered and declined: a bare collection display commonly holds side-effecting calls, so flagging it by outer type raises false positives without a documented bug to justify it.

## 2026-06-19 - Phase 2A: corpus-size checker (required) + iot fix

- Added `tools/corpus_size_checker.py` + `make corpus_size` (REQUIRED via `tests/test_corpus_size_checker.py`): proves each `Teeth.corpus_size` counts the collection its `prove()` actually judges, not an unrelated input. Motivated by `core/iot`, which declared `corpus_size=len(STREAM)`=19 (raw telemetry messages) while `prove()` makes a single aggregate comparison — one scenario advertised as nineteen.
- A naive "`corpus_size == len(corpus)`" gate is vacuous-green: every harness already writes `corpus_size=len(<CORPUS>)`, so the equality holds by construction (survey: ZERO bare-integer corpus_size literals across 69 harnesses). So the gate (approach (b), Scott-approved) ANCHORS the count to the proof path instead: resolve `corpus_size` to its collection name(s) (unwrap `len`/`sum`, follow a one-hop module constant), then walk prove's call graph for the names it ITERATES (`for`/comprehension) or COMPARES against (`==`/`!=`), with local-alias resolution — but NOT names merely passed as call arguments. OK if they intersect; MISLABELED if prove anchors elsewhere; UNANALYZABLE (advisory) if the size is dynamic (`len(runner._combos(...))`) or prove has no static anchor.
- KEY BUG caught in my own checker during the survey: the alias pass first treated `got = impl(STREAM, READINGS)` as "`got` aliases `STREAM`", so STREAM leaked into the anchors and iot read OK. Fixed by restricting aliases to pure name pass-throughs (`a = b`, `a = b if c else d`, `a = b or c`) via `_passthrough_names` — a call result is never an alias of its arguments. After the fix: 67 anchored, 1 MISLABELED (iot), 1 UNANALYZABLE (feature_flag's runner-enumerated matrix). The compare-operand + alias resolution is what clears pagination/pipeline (compare against `_EXPECTED_IDS`/`AGG_CORPUS`) and grpc (`rpcs = RPCS` alias inside `audit()`).
- Fixed iot: `corpus_size=len(STREAM)` -> `corpus_size=1` (one aggregate scenario; each mutant perturbs a different fingerprint dimension). Regenerated `cards/cards.json` + `cards/CARDS.md` (`cards/harness_card.py --write`) — a 2-line diff, iot 19->1. Now 67 anchored / 0 mislabeled / 2 unanalyzable; UNANALYZABLE is advisory so the required gate passes. file_guard now 15 protected.
