...
- ADVISORY per DP3: `tests/test_dead_expr_checker.py` asserts only the checker's own fixture behavior plus that it analyses every harness without raising — it does NOT assert repo-wide cleanliness, so the gate never red-locks main. It is the forward-looking tripwire for new harnesses (#78+); ratchet to required is a Phase 6 option once FP-confidence holds across all harnesses and the five CI Pythons.
