...
## 2026-06-17 - docs reader layers for humans and AI

- Added `docs/HARNESS_READING_GUIDE.md` as the descriptive human/AI reading path. It is not an instruction source; `AGENTS.md`, `CLAUDE.md`, and `SECURITY.md` remain the operating boundary.
- Added root `llms.txt` as a compact navigation map. It is descriptive and public, not access control and not a crawler/security policy.
- Design choice: keep AGENTS lean and put harness-dossier/context expansion in normal docs. Current 2026 evidence warns that bloated repository context files can raise token cost and create context-bloat / skill-leakage / conflicting-instruction smells.
- Dossier shape established for later per-harness expansion: name, path, category, failure class, oracle, planted mutants, corpus/fixtures, proof status, vacuity targets, commands, limits, related harnesses.
- Next useful layer is a mechanically generated or checked per-harness dossier index; avoid hand-maintained counts that drift.
