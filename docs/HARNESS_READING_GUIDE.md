...
| 5. Gate machinery | Maintaining the proof system itself | `Makefile`, `tools/proof_audit.py`, `.github/control-policy.json`, gate tools documented in the repo |
...
4. Open the harness file and its paired test.
5. Run the smallest relevant command first:
...
6. Use `docs/LEARNINGS.md` only for gotchas and historical context. Do not treat it as canonical if live code disagrees.
...
4. For gate machinery, run the specific gate being changed and do not claim CI green until CI reports green.
...
5. `HARNESS_INVENTORY.md` and `HARNESS_ROADMAP.md` to avoid duplicate harness work.
...
The next useful layer is a per-harness dossier index derived from `HARNESS_INVENTORY.md` plus live TEETH status. Build it as a generated or mechanically checkable artifact if possible; avoid hand-maintaining counts that can drift.
